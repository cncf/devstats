...
kubectl exec -itn devstats-prod devstats-postgres-$N -- pg_dump $DB --table gha_events > "${DB}_gha_events.sql"
...
kubectl exec -in devstats-prod devstats-postgres-$N -- psql "${DB}_tmp" < "${DB}_gha_events.sql"
rm -f "${DB}_gha_repos.sql" "${DB}_gha_repo_groups.sql" "${DB}_gha_events.sql"
