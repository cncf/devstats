...
./devel/db.sh psql ko -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh ko || exit 1
...
GHA2DB_PROJECT=ko PG_DB=ko ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
