...
./shared/setup_shared_fdw.sh ovnkubernetes || exit 1
./devel/ro_user_grants.sh ovnkubernetes || exit 2
GHA2DB_PROJECT=ovnkubernetes PG_DB=ovnkubernetes GHA2DB_LOCAL=1 gha2db 2016-08-11 0 today now 'ovn-org,ovn-kubernetes,openvswitch/ovn-kubernetes' 2>>errors.txt | tee -a run.log || exit 3
...
GHA2DB_PROJECT=ovnkubernetes PG_DB=ovnkubernetes ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
