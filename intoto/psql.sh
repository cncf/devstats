...
./devel/db.sh psql intoto -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh intoto || exit 1
...
GHA2DB_PROJECT=intoto PG_DB=intoto ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
