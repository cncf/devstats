...
./devel/db.sh psql chubaofs -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh chubaofs || exit 1
...
GHA2DB_PROJECT=chubaofs PG_DB=chubaofs ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
