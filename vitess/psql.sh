...
./devel/db.sh psql vitess -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh vitess || exit 1
...
GHA2DB_PROJECT=vitess PG_DB=vitess ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
