...
./devel/db.sh psql k8up -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh k8up || exit 1
...
GHA2DB_PROJECT=k8up PG_DB=k8up ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
