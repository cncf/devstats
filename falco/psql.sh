...
./devel/db.sh psql falco -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh falco || exit 1
...
GHA2DB_PROJECT=falco PG_DB=falco ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
