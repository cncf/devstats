...
./devel/db.sh psql linkerd -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh linkerd || exit 1
...
GHA2DB_PROJECT=linkerd PG_DB=linkerd ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
