...
- Set `GHA2DB_RECONCILE_HIST`, `reconcile_dbs` tool - use the project's `hist_command_line` from `projects.yaml` (the widest org/repo scope the project ever had) instead of `command_line` to decide which repositories' events are pulled; projects without `hist_command_line` fall back to `command_line`. For one-off backfills, the daily sync keeps the current rules.
...
- Set `GHA2DB_GHAPISKIPREPOEVENTS`, ghapi2db tool, skip the repository events feed pass (`GET /repos/{owner}/{repo}/events`, up to 3 pages of 100 per repository with heartbeat activity, written with the gha2db writer under the events' native ids - fills the events GH Archive missed; the pass applies the project's own org/repo/actor ingestion rules from `projects.yaml`, repositories outside them are not asked for their feed), default false (feed pass enabled).
- Set `GHA2DB_GHAPISKIPISSUESPRS`, ghapi2db tool, skip the issues/PRs sweep pass (upgrades the stub `gha_pull_requests` rows GH Archive delivers since 2024-10 - `created_at = 0001-01-01` - with `GET /repos/{owner}/{repo}/pulls/{n}` objects, attaches missing `gha_issues` rows, and synthesizes GHA-shaped `opened`/`closed` `IssuesEvent`/`PullRequestEvent` rows from `GET /repos/{owner}/{repo}/issues?since=...` for the issues and PRs the database does not know at all), default false (sweep enabled).
