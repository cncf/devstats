...
./devel/db.sh psql opentracing -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh opentracing || exit 1
...
GHA2DB_PROJECT=opentracing PG_DB=opentracing ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
