...
./devel/db.sh psql istio -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh istio || exit 1
...
GHA2DB_PROJECT=istio PG_DB=istio ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
