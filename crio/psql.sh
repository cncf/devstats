...
./devel/db.sh psql crio -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh crio || exit 1
...
GHA2DB_PROJECT=crio PG_DB=crio ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
