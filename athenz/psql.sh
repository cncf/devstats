...
./devel/db.sh psql athenz -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh athenz || exit 1
...
GHA2DB_PROJECT=athenz PG_DB=athenz ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 7
