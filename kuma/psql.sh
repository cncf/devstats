...
./devel/db.sh psql kuma -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh kuma || exit 1
...
GHA2DB_PROJECT=kuma PG_DB=kuma ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
