...
./devel/db.sh psql linux -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh linux || exit 1
...
GHA2DB_PROJECT=linux PG_DB=linux ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
