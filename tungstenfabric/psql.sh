...
./devel/db.sh psql tungstenfabric -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh tungstenfabric || exit 1
...
GHA2DB_PROJECT=tungstenfabric PG_DB=tungstenfabric ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
