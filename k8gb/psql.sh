...
./devel/db.sh psql k8gb -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh k8gb || exit 1
...
GHA2DB_PROJECT=k8gb PG_DB=k8gb ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
