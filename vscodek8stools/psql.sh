...
./devel/db.sh psql vscodek8stools -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh vscodek8stools || exit 1
...
GHA2DB_PROJECT=vscodek8stools PG_DB=vscodek8stools ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
