...
./devel/db.sh psql fluentd -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh fluentd || exit 1
...
GHA2DB_PROJECT=fluentd PG_DB=fluentd ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 7
