...
- Currently it is defined as `not like all(array[...])` with exact (lower case) bot logins followed by `like` wildcard patterns, for example: `not like all(array['googlebot', 'coveralls', 'rktbot', 'coreosbot', 'web-flow', 'copilot%', 'dependabot%', 'k8s-%', '%-bot', '%bot-%', '%-robot', '%[%bot]%', '%-jenkins', '%ci%bot', '%-testing', 'codecov%', '%automat%', '%agent', '% bot'])` - see the file for the full list (123 patterns, last audited against live `allprj` data on 2026-09-13).
