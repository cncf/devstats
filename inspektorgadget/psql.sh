...
./devel/db.sh psql inspektorgadget -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh inspektorgadget || exit 1
...
GHA2DB_PROJECT=inspektorgadget PG_DB=inspektorgadget ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
