...
./devel/db.sh psql clusternet -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh clusternet || exit 1
...
GHA2DB_PROJECT=clusternet PG_DB=clusternet ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
