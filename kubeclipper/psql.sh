...
./devel/db.sh psql kubeclipper -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh kubeclipper || exit 1
...
GHA2DB_PROJECT=kubeclipper PG_DB=kubeclipper ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
