...
./devel/db.sh psql opencontainers -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh opencontainers || exit 1
...
GHA2DB_PROJECT=opencontainers PG_DB=opencontainers ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
