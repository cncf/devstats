...
./devel/db.sh psql telepresence -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh telepresence || exit 1
...
GHA2DB_PROJECT=telepresence PG_DB=telepresence ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
