...
./devel/db.sh psql grpc -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh grpc || exit 1
...
GHA2DB_PROJECT=grpc PG_DB=grpc ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
