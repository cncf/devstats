...
./devel/db.sh psql harbor -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh harbor || exit 1
...
GHA2DB_PROJECT=harbor PG_DB=harbor ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
