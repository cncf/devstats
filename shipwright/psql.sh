...
./devel/db.sh psql shipwright -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh shipwright || exit 1
...
GHA2DB_PROJECT=shipwright PG_DB=shipwright ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
