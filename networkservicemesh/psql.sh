...
./devel/db.sh psql networkservicemesh -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh networkservicemesh || exit 1
...
GHA2DB_PROJECT=networkservicemesh PG_DB=networkservicemesh ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
