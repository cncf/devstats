...
./devel/db.sh psql azf -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh azf || exit 1
...
GHA2DB_PROJECT=azf PG_DB=azf ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
