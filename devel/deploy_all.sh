...
do
  ...
  then
    PROJ=networkservicemesh  PROJDB=networkservicemesh PROJREPO="networkservicemesh/api"      ORGNAME="Network Service Mesh" PORT=3035 ICON=networkservicemesh GRAFSUFF=networkservicemesh GA="UA-108085315-46" ./devel/deploy_proj.sh || exit 45
  elif [ "$proj" = "openebs" ]
  then
  ...
  then
    PROJ=konveyor            PROJDB=konveyor       PROJREPO="konveyor/operator"               ORGNAME=Konveyor          PORT=3136 ICON=konveyor       GRAFSUFF=konveyor       GA="UA-145264316-97" ./devel/deploy_proj.sh || exit 55
  elif [ "$proj" = "armada" ]
  then
