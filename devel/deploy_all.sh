...
do
  ...
  then
    PROJ=serverlessworkflow PROJDB=serverlessworkflow PROJREPO="open-workflow-specification/specification" ORGNAME="Open Workflow Specification" PORT=3063 ICON=serverlessworkflow GRAFSUFF=serverlessworkflow GA="-" ./devel/deploy_proj.sh || exit 81
  elif [ "$proj" = "k3s" ]
  then
  ...
  then
    PROJ=kbind               PROJDB=kbind          PROJREPO="kbind-dev/kbind"                 ORGNAME=kbind             PORT=3328 ICON=kbind          GRAFSUFF=kbind          GA="-" ./devel/deploy_proj.sh               || exit 254
  elif [ "$proj" = "curvine" ]
  then
