...
./devel/db.sh psql zot -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh zot || exit 1
...
GHA2DB_PROJECT=zot PG_DB=zot ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
