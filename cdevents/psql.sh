...
./devel/db.sh psql cdevents -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh cdevents || exit 1
...
GHA2DB_PROJECT=cdevents PG_DB=cdevents ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
