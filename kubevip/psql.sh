...
./devel/db.sh psql kubevip -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh kubevip || exit 1
...
GHA2DB_PROJECT=kubevip PG_DB=kubevip ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
