...
./devel/db.sh psql kubeflow -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh kubeflow || exit 1
...
GHA2DB_PROJECT=kubeflow PG_DB=kubeflow ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
