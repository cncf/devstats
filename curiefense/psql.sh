...
./devel/db.sh psql curiefense -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh curiefense || exit 1
...
GHA2DB_PROJECT=curiefense PG_DB=curiefense ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
