...
./devel/db.sh psql nats -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh nats || exit 1
...
GHA2DB_PROJECT=nats PG_DB=nats ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
