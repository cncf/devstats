...
./devel/db.sh psql antrea -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh antrea || exit 1
...
GHA2DB_PROJECT=antrea PG_DB=antrea ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
