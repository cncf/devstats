...
./devel/db.sh psql kubeovn -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh kubeovn || exit 1
...
GHA2DB_PROJECT=kubeovn PG_DB=kubeovn ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
