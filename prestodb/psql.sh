...
./devel/db.sh psql prestodb -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh prestodb || exit 1
...
GHA2DB_PROJECT=prestodb PG_DB=prestodb ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
