...
./devel/db.sh psql opcr -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh opcr || exit 1
...
GHA2DB_PROJECT=opcr PG_DB=opcr ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
