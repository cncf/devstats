...
./devel/db.sh psql zephyr -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh zephyr || exit 1
...
GHA2DB_PROJECT=zephyr PG_DB=zephyr ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
