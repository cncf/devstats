...
./devel/db.sh psql chaosmesh -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh chaosmesh || exit 1
...
GHA2DB_PROJECT=chaosmesh PG_DB=chaosmesh ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
