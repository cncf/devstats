...
./devel/db.sh psql graphqlspec -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh graphqlspec || exit 1
...
GHA2DB_PROJECT=graphqlspec PG_DB=graphqlspec ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
