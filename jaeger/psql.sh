...
./devel/db.sh psql jaeger -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh jaeger || exit 1
...
GHA2DB_PROJECT=jaeger PG_DB=jaeger ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
