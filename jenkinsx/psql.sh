...
./devel/db.sh psql jenkinsx -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh jenkinsx || exit 1
...
GHA2DB_PROJECT=jenkinsx PG_DB=jenkinsx ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
