...
./devel/db.sh psql etcd -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh etcd || exit 1
...
GHA2DB_PROJECT=etcd PG_DB=etcd ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
