...
./devel/db.sh psql cortex -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh cortex || exit 1
...
GHA2DB_PROJECT=cortex PG_DB=cortex ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
