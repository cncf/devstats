...
./devel/db.sh psql hwameistor -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh hwameistor || exit 1
...
GHA2DB_PROJECT=hwameistor PG_DB=hwameistor ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
