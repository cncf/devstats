...
./shared/setup_shared_fdw.sh modelpack || exit 1
./devel/ro_user_grants.sh modelpack || exit 2
GHA2DB_PROJECT=modelpack PG_DB=modelpack GHA2DB_LOCAL=1 gha2db 2024-08-28 0 today now 'CloudNativeAI,modelpack' 2>>errors.txt | tee -a run.log || exit 3
...
GHA2DB_PROJECT=modelpack PG_DB=modelpack ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
