...
./devel/db.sh psql thanos -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh thanos || exit 1
GHA2DB_PROJECT=thanos PG_DB=thanos GHA2DB_LOCAL=1 gha2db 2017-11-01 0 today now 'thanos-io,thanos-community,improbable-eng/promlts,improbable-eng/thanos' 2>>errors.txt | tee -a run.log || exit 2
...
GHA2DB_PROJECT=thanos PG_DB=thanos ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
