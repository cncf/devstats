...
./devel/db.sh psql tremor -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh tremor || exit 1
...
GHA2DB_PROJECT=tremor PG_DB=tremor ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
