...
./devel/db.sh psql helm -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh helm || exit 1
...
GHA2DB_PROJECT=helm PG_DB=helm ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
