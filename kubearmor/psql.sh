...
./devel/db.sh psql kubearmor -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh kubearmor || exit 1
...
GHA2DB_PROJECT=kubearmor PG_DB=kubearmor ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
