...
./devel/db.sh psql k3s -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh k3s || exit 1
...
GHA2DB_PROJECT=k3s PG_DB=k3s ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
