...
./devel/db.sh psql k8dash -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh k8dash || exit 1
...
GHA2DB_PROJECT=k8dash PG_DB=k8dash ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
