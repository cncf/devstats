...
./devel/db.sh psql tuf -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh tuf || exit 1
...
GHA2DB_PROJECT=tuf PG_DB=tuf ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
