...
./devel/db.sh psql containerssh -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh containerssh || exit 1
...
GHA2DB_PROJECT=containerssh PG_DB=containerssh ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
