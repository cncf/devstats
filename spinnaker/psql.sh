...
./devel/db.sh psql spinnaker -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh spinnaker || exit 1
...
GHA2DB_PROJECT=spinnaker PG_DB=spinnaker ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
