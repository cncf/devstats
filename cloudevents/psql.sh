...
./devel/db.sh psql cloudevents -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh cloudevents || exit 1
...
GHA2DB_PROJECT=cloudevents PG_DB=cloudevents ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
