...
./devel/db.sh psql hyperledger -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh hyperledger || exit 1
...
GHA2DB_PROJECT=hyperledger PG_DB=hyperledger ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 8
