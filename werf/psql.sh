...
./devel/db.sh psql werf -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh werf || exit 1
...
GHA2DB_PROJECT=werf PG_DB=werf ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
