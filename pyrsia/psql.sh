...
./devel/db.sh psql pyrsia -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh pyrsia || exit 1
...
GHA2DB_PROJECT=pyrsia PG_DB=pyrsia ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
