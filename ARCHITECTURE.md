...
- The repository events feed pass (`GET /repos/{owner}/{repo}/events`, fills the events GH Archive missed) applies the project's own org/repo/actor ingestion rules from `projects.yaml` (`command_line`, `GHA2DB_EXCLUDE_REPOS`, `GHA2DB_EXACT`, actors filter - the same rules `gha2db` uses for that project): repositories outside them are not asked for their feed and feed events recorded under a former repository name are dropped, so a repository that moved out of the project's organization does not get its current events restored into the project database. Reported as `ghapi2db repo events: filter: ...` and `... filtered out N repo(s) and N event(s) ...` lines.
...
- A database is only written by its own sync (pull only): a project database pulls from its `shared_db`, the shared database pulls from every project database sharing it, `GHA2DB_RECONCILE_DBS` lists explicit sources. Repository scope: repos present in both `gha_repos`, further narrowed by the target project's own ingestion rules from `projects.yaml` (`command_line` org/repo lists or `regexp:` patterns, `GHA2DB_EXCLUDE_REPOS`, `GHA2DB_EXACT`, actors filter — exactly what `gha2db` applies for that project), so a shared database never leaks events of a repository the project does not track (for example a repository moved to another organization); time window: `GHA2DB_RECONCILE_RANGE` (default '90 days').
...
- With `GHA2DB_RECONCILE_HIST=1` the repository scope uses the project's `hist_command_line` from `projects.yaml` instead of `command_line`: the widest org/repo scope the project ever had (every organization and repository it tracked at any time, generated 2026-09 from the projects' history; projects without it fall back to `command_line`). Use it for one-off backfills of the events that were ingested under former repository names/organizations - the daily sync keeps the current rules.
