...
./devel/db.sh psql opensecuritycontroller -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh opensecuritycontroller || exit 1
...
GHA2DB_PROJECT=opensecuritycontroller PG_DB=opensecuritycontroller ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 9
