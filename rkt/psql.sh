...
./devel/db.sh psql rkt -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh rkt || exit 1
...
GHA2DB_PROJECT=rkt PG_DB=rkt ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 7
