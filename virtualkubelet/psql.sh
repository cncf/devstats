...
./devel/db.sh psql virtualkubelet -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh virtualkubelet || exit 1
...
GHA2DB_PROJECT=virtualkubelet PG_DB=virtualkubelet ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
