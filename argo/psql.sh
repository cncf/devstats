...
./devel/db.sh psql argo -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh argo || exit 1
...
GHA2DB_PROJECT=argo PG_DB=argo ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
