...
./devel/db.sh psql prometheus -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh prometheus || exit 1
GHA2DB_PROJECT=prometheus PG_DB=prometheus GHA2DB_LOCAL=1 gha2db 2015-01-01 0 today now 'prometheus,OpenObservability,OpenMetrics,openmetrics,prometheus-community,RichiH/OpenMetrics' 2>>errors.txt | tee -a run.log || exit 2
...
GHA2DB_PROJECT=prometheus PG_DB=prometheus ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 7
