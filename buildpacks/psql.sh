...
./devel/db.sh psql buildpacks -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh buildpacks || exit 1
GHA2DB_PROJECT=buildpacks PG_DB=buildpacks GHA2DB_LOCAL=1 gha2db 2018-06-01 0 today now 'buildpack,buildpacks,buildpacks-community' 2>>errors.txt | tee -a run.log || exit 2
...
GHA2DB_PROJECT=buildpacks PG_DB=buildpacks ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
