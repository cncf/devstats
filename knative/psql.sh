...
./shared/setup_shared_fdw.sh knative || exit 1
GHA2DB_PROJECT=knative PG_DB=knative GHA2DB_LOCAL=1 gha2db  2018-07-23 0 today now 'knative,knative-sandbox,knative-extensions' 2>>errors.txt | tee -a run.log || exit 2
...
GHA2DB_PROJECT=knative PG_DB=knative ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
