...
for proj in $all
do
  if ( [ "$proj" = "opencontainers" ] || [ "$proj" = "cdf" ] || [ "$proj" = "prestodb" ] || [ "$proj" = "godotengine" ] || [ "$icon" = "linux" ] )
  then
    continue
  fi
  suff=$proj
