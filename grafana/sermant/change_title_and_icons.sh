#!/bin/bash
# GRAFANA_DATA=/usr/share/grafana.sermant/
...
then
  ...
  do
    ls -l "$f"
    sed -i "s|'Grafana - '|'Sermant DevStats - '|g" "$f"
  done
  for f in `find ${GRAFANA_DATA} -type f -exec grep -l '"Grafana - "' "{}" \; | sort | uniq`
  do
    ls -l "$f"
