#!/bin/bash
# GRAFANA_DATA=/usr/share/grafana.kbind/
...
then
  ...
  do
    ls -l "$f"
    sed -i "s|'Grafana - '|'kbind DevStats - '|g" "$f"
  done
  for f in `find ${GRAFANA_DATA} -type f -exec grep -l '"Grafana - "' "{}" \; | sort | uniq`
  do
    ls -l "$f"
