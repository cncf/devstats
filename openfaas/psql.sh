...
./devel/db.sh psql openfaas -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh openfaas || exit 1
...
GHA2DB_PROJECT=openfaas PG_DB=openfaas ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
