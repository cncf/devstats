...
./devel/db.sh psql rook -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh rook || exit 1
...
GHA2DB_PROJECT=rook PG_DB=rook ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
