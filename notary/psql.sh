...
./devel/db.sh psql notary -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh notary || exit 1
...
GHA2DB_PROJECT=notary PG_DB=notary ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
