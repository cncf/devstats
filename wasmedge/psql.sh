...
./devel/db.sh psql wasmedge -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh wasmedge || exit 1
...
GHA2DB_PROJECT=wasmedge PG_DB=wasmedge ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
