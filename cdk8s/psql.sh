...
./devel/db.sh psql cdk8s -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh cdk8s || exit 1
...
GHA2DB_PROJECT=cdk8s PG_DB=cdk8s ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
