...
./devel/db.sh psql gha -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh gha || exit 1
...
GHA2DB_PROJECT=kubernetes PG_DB=gha ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 8
