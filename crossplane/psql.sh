...
./devel/db.sh psql crossplane -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh crossplane || exit 1
...
GHA2DB_PROJECT=crossplane PG_DB=crossplane ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
