...
./devel/db.sh psql servicemeshperformance -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh servicemeshperformance || exit 1
...
GHA2DB_PROJECT=servicemeshperformance PG_DB=servicemeshperformance ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
