...
./devel/db.sh psql kubevirt -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh kubevirt || exit 1
GHA2DB_PROJECT=kubevirt PG_DB=kubevirt GHA2DB_LOCAL=1 gha2db 2016-04-27 0 today now 'kubevirt,k8snetworkplumbingwg/kubemacpool,k8snetworkplumbingwg/multi-networkpolicy-iptables,k8snetworkplumbingwg/sriov-network-operator,nmstate/kubernetes-nmstate' 2>>errors.txt | tee -a run.log || exit 2
...
GHA2DB_PROJECT=kubevirt PG_DB=kubevirt ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
