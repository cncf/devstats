...
./devel/db.sh psql tinkerbell -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh tinkerbell || exit 1
...
GHA2DB_PROJECT=tinkerbell PG_DB=tinkerbell ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
