...
./devel/db.sh psql smi -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh smi || exit 1
...
GHA2DB_PROJECT=smi PG_DB=smi ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
