...
./devel/db.sh psql spiffe -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh spiffe || exit 1
...
GHA2DB_PROJECT=spiffe PG_DB=spiffe ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
