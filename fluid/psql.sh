...
./devel/db.sh psql fluid -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh fluid || exit 1
...
GHA2DB_PROJECT=fluid PG_DB=fluid ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
