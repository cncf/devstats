...
./devel/db.sh psql porter -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh porter || exit 1
...
GHA2DB_PROJECT=porter PG_DB=porter ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
