...
./devel/db.sh psql openclustermanagement -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh openclustermanagement || exit 1
...
GHA2DB_PROJECT=openclustermanagement PG_DB=openclustermanagement ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
