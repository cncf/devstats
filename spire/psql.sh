...
./devel/db.sh psql spire -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh spire || exit 1
...
GHA2DB_PROJECT=spire PG_DB=spire ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
