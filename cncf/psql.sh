...
./devel/db.sh psql cncf -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh cncf || exit 1
GHA2DB_PROJECT=cncf PG_DB=cncf GHA2DB_LOCAL=1 gha2db 2015-12-07 18 today now 'cncf,crosscloudci,cdfoundation,cncf-tags' 2>>errors.txt | tee -a run.log || exit 2
...
GHA2DB_PROJECT=cncf PG_DB=cncf ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
