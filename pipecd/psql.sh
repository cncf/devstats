...
./shared/setup_shared_fdw.sh pipecd || exit 1
...
GHA2DB_PROJECT=pipecd PG_DB=pipecd ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
GHA2DB_PROJECT=pipecd PG_DB=pipecd ./shared/setup_scripts.sh 2>>errors.txt | tee -a run.log || exit 6
GHA2DB_PROJECT=pipecd PG_DB=pipecd ./shared/get_repos.sh 2>>errors.txt | tee -a run.log || exit 7
GHA2DB_PROJECT=pipecd PG_DB=pipecd GHA2DB_LOCAL=1 vars || exit 8
./devel/ro_user_grants.sh pipecd || exit 9
./devel/psql_user_grants.sh devstats_team pipecd || exit 10
