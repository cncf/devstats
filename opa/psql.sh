...
./devel/db.sh psql opa -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh opa || exit 1
...
GHA2DB_PROJECT=opa PG_DB=opa ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
