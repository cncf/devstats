...
./devel/db.sh psql jenkins -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh jenkins || exit 1
...
GHA2DB_PROJECT=jenkins PG_DB=jenkins ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
