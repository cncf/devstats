...
./devel/db.sh psql lima -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh lima || exit 1
...
GHA2DB_PROJECT=lima PG_DB=lima ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 5
