...
./devel/db.sh psql cni -c "create extension if not exists hll" || exit 1
./shared/setup_shared_fdw.sh cni || exit 1
...
GHA2DB_PROJECT=cni PG_DB=cni ./shared/proj_affs_bootstrap.sh 2>>errors.txt | tee -a run.log || exit 6
